...
class TestHelpdesk(TransactionCase):
    def setUp(self):
        ...
        self.ticket = self.env.ref('helpdesk_enriquelopez.helpdesk_ticket_demo_01')
        self.tag = self.env.ref('helpdesk_enriquelopez.helpdesk_ticket_tag_demo_01')
        self.ticket.tag_ids = self.tag.ids

    def test_10_tag_assigned(self):
