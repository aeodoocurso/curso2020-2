...
class NewTicketFromTag(models.TransientModel):
    ...
    def create_ticket(self):
        # tag_ids = self.context.get('active_id')
        ...
        action = self.env.ref('helpdesk_enriquelopez.helpdesk_ticket_action').read()[0]
        action['context'] = {'default_tag_ids':[(6, 0, self.tag_id.ids)],}
        # action['domain'] = [('tag_ids', '=', self.id)]
        action['views'] = [(self.env.ref('helpdesk_enriquelopez.view_helpdesk_ticket_form').id, 'form')]
